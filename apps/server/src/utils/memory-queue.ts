 ...
 *
 *  Повышение версии НЕ переразбирает уже обработанные главы, хотя строки и
 *  уникальны по (chapter_version_id, kind, pipeline_version). Задания ставятся
 *  только на свежесозданную версию главы, а у неё строк нет ни под какой
 *  версией — уникальность тут ничего не открывает. Единственный путь к
 *  повторному разбору прежних глав — «Перестроить память с главы N», который
 *  удаляет строки commit-видов и ставит их заново.
 *
 *  Отсюда следствие, которое надо помнить: после выпуска события появляются
 *  только у глав, зафиксированных ПОСЛЕ него. У книги, написанной раньше, их
 *  не будет, пока автор не перестроит память. Экран главы теперь об этом
 *  говорит: `outdatedPipelineChapters` считает такие главы.
 *
 *  И ещё одно, что легко принять наоборот: `pipeline_version` НЕ описывает
 *  форму `result_json`. Задание версии 1, брошенное падением в `running`,
 *  восстановление переводит в `retry`, и доделывает его уже нынешний
 *  обработчик — результат новой формы ложится под старым номером. Читать
 *  надо сами поля результата, а не судить о них по колонке. */
