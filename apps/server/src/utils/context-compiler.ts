...
export function requiredOverflowMessage(compiled: CompiledContext): string {
  // Действие называется выполнимое. Состав сцены автор не задаёт руками —
  // он выводится из намерения главы и её беат-листа, и сократить его
  // значит убрать героя оттуда (С1).
  return (
    `обязательный контекст сцены (~${compiled.requiredTokens} токенов) не помещается ` +
    `в бюджет (${compiled.budgetTokens}). В нём карточки участников с их знаниями и ` +
    `действующими фактами. Уберите лишних героев из намерения главы и беат-листа ` +
    `или разбейте сцену на две главы.`
  );
}
