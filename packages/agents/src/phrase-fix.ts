...
Для каждой фразы верни: id; before — точный кусок абзаца, который заменяешь, символ в символ: отмеченная фраза целиком или она вместе с одним соседним предложением; after — чем его заменить (пустая строка — удалить). Если фразу нельзя исправить, не испортив смысл, — не включай её в ответ. Не трогай и то, что говорит о герое, а не украшает:
...
- пояснение, которое меняет смысл действия, а не повторяет его.
Такую фразу не включай в ответ, даже если она отмечена.`;
