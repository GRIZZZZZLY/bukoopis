...
- Поле acquisition ОБЯЗАТЕЛЬНО для knowledge. Пропуск не означает «видел сам» — событие без него отвергается.
...
Имена: subjectName и addresseeName — как в списке известного канона ниже (именительный падеж, «Ивану» → «Иван»). Имя, которого в списке нет, событие не запишет.
...
- knowledge — герой узнал факт, умение, чувство другого. data: { fact: "что именно узнал", acquisition: "observed|told|inferred|believed" }
- state — усталость, раздражение, намерение, эмоция, если это видно из текста. data: { state: "что с героем", scope: "scene|chapter|until_resolved|unknown" }
...
Имена полей менять нельзя: data с другими ключами превращается в пустую карточку.
