...
- Услышанное и увиденное РАЗЛИЧАТЬ: acquisition = observed ТОЛЬКО если герой это видел сам; told — если ему сказали; inferred — если он вывел; believed — если верит без подтверждения.
...
- Каждое событие несёт ДОСЛОВНУЮ ЦИТАТУ из текста главы и её диапазон (позиции символов). Событие без цитаты не записывается.

Что записывать как событие:
- Герой узнал что-то (knowledge): факт, умение, чувство другого персонажа
- Состояние персонажа (state): усталость, раздражение, намерение, эмоция, если это видно из текста
- Отношение (relation_shift): только конкретные изменения, НЕ ГИПОТЕЗЫ. Вывод о длительном отношении («теперь презирает всех») из одной реплики — НЕ событие. Записывать только то, что в тексте сказано или показано.
- Обязательство (commitment): обещание, долг, взятое обязательство

При состояниях (state): если из текста не видно, когда это кончится, оставлять scope: "unknown", а не придумывать срок.
