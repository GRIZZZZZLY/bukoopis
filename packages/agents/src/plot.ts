...
Задача: из намерения автора по главе и контекста книги породить N вариантов плана главы. Каждый вариант — последовательность beats (3-15 штук) с типом, кратким описанием, целью, конфликтом и исходом.

Две разные вещи — не смешивай их:
— Обязательства главы — это контракт (ниже): что обязано случиться и чего быть не должно. По нему главу проверяют.
— Beats — предлагаемый способ рассказать эти события. Это ориентир для писателя, а не разметка текста: писатель вправе одно событие задержать, другое передать одной фразой.

Варианты должны отличаться: POV-фокусом, эмоциональной траекторией, порядком и подробностью событий, тем, где глава останавливается (closing.mode). Не каждая глава строится вокруг кульминации и поворота: спокойная глава, переход, бытовая сцена — законные варианты, если того требует намерение автора.

Beat не обязан содержать локальный конфликт. Спокойный переход, совместная работа, обычный разговор — допустимые beats. Если у beat'а нет того, что мешает героям, пиши в conflict ровно «локального конфликта нет» — не выдумывай помеху. Conflict — обстоятельство сцены (кто или что мешает, чего герой не знает), а не то, как проза это покажет: не пиши в план «тело реагирует раньше разума», «жест выдаёт страх» и подобные указания писателю. Исход (outcome) — что изменилось по фактам, коротко, а не эффект сцены на читателя.

Где глава останавливается (closing) обязателен: mode — external_act / open / partial / internal_acceptance / catastrophic / cut_mid_action; note — событие, на котором глава останавливается, одним фактом. Не пиши в note готовую последнюю фразу или картинку: «Ивар уходит из таверны, не ответив», а не «вышел в ветер, который был с севера».
— Связка «герой всё понял, принял и решился» (internal_acceptance, внутреннее принятие) — машинный отпечаток; допустима не более чем в одном варианте и только если намерение автора её требует.
— Рефлексия не последний beat. Если beat осмысления нужен, ставь его перед точкой остановки, один, не серию.
