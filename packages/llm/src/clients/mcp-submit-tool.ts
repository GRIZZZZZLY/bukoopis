...
/**
 * Экранирует двойные кавычки, стоящие ВНУТРИ строкового значения JSON.
 *
 * Живой прогон 2026-09-20: строка с событиями не разбиралась не потому, что
 * оборвана, а потому, что в дословной цитате стоят кавычки — «техник
 * насосной станции "Северная"». Модель копирует цитату посимвольно, как от
 * неё и требуют, но, сериализуя массив в строку вручную, внутренние кавычки
 * не экранирует. На реальной главе так терялись все пятнадцать событий.
 *
 * Закрывающей считается кавычка, за которой (через пробелы) идёт `,`, `:`,
 * `}` или `]` — то есть та, после которой JSON продолжается структурой.
 * Любая другая экранируется. Эвристика, но проверяемая: разбор всё равно
 * идёт через `JSON.parse`, и неудача остаётся неудачей.
 */
 ...
 *
 * Строка, которая начинается как JSON-массив, но не разбирается, сначала
...
function parseJsonStringFields(args: unknown): { args: unknown; lost: string[] } {
  ...
  for (const [key, value] of Object.entries(out)) {
    ...
    try {
      ...
      try {
        const repaired: unknown = JSON.parse(repairUnescapedQuotes(trimmed));
