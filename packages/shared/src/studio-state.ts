...
export const stageStateSchema = z.object({
   ...
   *  разово, потому что сборка идёт минутами и терять набранное при
   *  обновлении страницы нельзя — они уходят в каждую сборку (кнопку и
   *  быстрый сбор). «Переписать»/«Другие варианты» отдельного раздела их не
   *  видят: `aspect_refine` и `aspect_variants` заметок не читают. */
